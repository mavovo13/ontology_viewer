...
**責務**: カラーテーマの切り替えUIを制御し、選択テーマをlocalStorageに永続化する

```javascript
// サポートテーマ
const THEMES = [
  { id: 'nostromo', name: 'NOSTROMO' },  // デフォルト（CRT緑）
  { id: 'amber',    name: 'AMBER'    },  // アンバー（CRT琥珀）
  { id: 'folio',    name: 'FOLIO'    },  // フォリオ（淡色）
  { id: 'void',     name: 'VOID'     },  // ボイド（深黒）
];

class ThemeSwitcher {
  constructor(containerEl);

  // テーマを切り替える（CRTフリッカーアニメーション付き）
  applyTheme(themeId /* string */);
}
```

**依存関係**: AppState非依存。`localStorage` と `document.documentElement.dataset.theme` で状態管理。

**テーマ切り替えの仕組み**:
- `<html data-theme="[id]">` 属性の付け替えでCSSテーマを切り替える
- デフォルト（nostromo）は属性なし
- 切り替え時にCRTフリッカーオーバーレイアニメーションを再生（210ms後にテーマ適用）
- 選択テーマは `localStorage` キー `ontology-viewer-theme` に保持
...
```
┌──────────────────────────────────────────────────────────────────────┐
│  Header: Ontology Viewer  [W_CONCEPTS | R_CONCEPTS]  THEME: ■ □ □ □  │
├──────────────────────────────────────────┬───────────────────────────┤
│  ▸ VIEWPORT W_CONCEPTS                   │  CONCEPT · READOUT    [✕] │
│                                          │  ─────────────────────── │
│   Graph Area (Cytoscape.js)              │  概念名: Human            │
│   ┌─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─┐       │  ラベル: 人間              │
│   │  (ドロップゾーン / グラフ)    │       │                          │
│   └─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─┘       │  スロット:                │
│                                          │  - name: string          │
│  NODES 012 ┃ EDGES 008  LAYOUT ELK       │  - age: int              │
│                               [FIT VIEW] │  ─────────────────────── │
│  [読み込み中ファイル名バー]               │                          │
└──────────────────────────────────────────┴───────────────────────────┘
```
...
**ヘッダー要素**:
- アプリタイトル（左）
- ビュー切り替えタブ（中央）: W_CONCEPTS / R_CONCEPTS
- テーマチップ（右）: NOSTROMO / AMBER / FOLIO / VOID

**グラフエリア内の補助UI**:
- `viewport-tag（左上）`: 現在表示中のビュー名（HUD）
- `viewport-tag（左下）`: ノード数・エッジ数（HUD）
- `viewport-tag（右下）`: レイアウトエンジン名（HUD）
- `reticle`: グラフ中央の照準レティクル（装飾）
- `file-name-bar`: 読み込み中のXMLファイル名表示バー（下部）
- `FIT VIEW` ボタン: グラフ全体をフィット表示
...
| 要素 | スタイル |
|------|---------|
| W_CONCEPTノード | 角丸長方形、ホスファー系（`--phos-500`）テーマに追従 |
| R_CONCEPTノード | 角丸長方形、アンバー系（`--amber-500`）テーマに追従 |
| ISAエッジ | 実線矢印、ミュートカラー |
| ARCエッジ | 実線矢印、アンバー系、ラベル付き |
| 選択中ノード | ボーダー強調（テーマのglowカラー） |
